...
pendulum3_max_speed = 8.0  # Added max speed for third pendulum
...
def animate_pendulum(run_time_seconds, direction1, direction2, direction3, reverse_main_pendulum, image_count, remaining_images):
    ...
    # Maximize the figure window (for Windows)
    ...
    def update(frame):
        ...
        if elapsed_time > run_time_seconds:
            ...
            image_counter_text.set_visible(False)
            plt.draw()
        ...
        image_counter_text.set_text(f"Images saved: {image_count}\nRemaining images: {remaining_images}")
...
def main():
    ...
    if enable_third_pendulum:
        current_direction3 = random.choice([1, -1])  # Random initial direction
    else:
        current_direction3 = None  # Set to None when third pendulum is disabled
    ...
    while True:
        ...
        # For direction2, check alternate_direction2
        if alternate_direction2:
            direction2 = current_direction2 * -1  # Reverse direction from previous run
        else:
            direction2 = current_direction2  # Keep the same direction
        if enable_third_pendulum and current_direction3 is not None:
            direction3 = current_direction3 * -1  # Reverse if third pendulum enabled
        else:
            direction3 = 0  # Or set to 0 when third pendulum is disabled
